...
/**
 * 访问统计（无参）
 * 返回 ResultMessage.data 为 VisitStatsDto[]
 */
...
/**
 * 耗时统计（无参）
 * 返回 ResultMessage.data 为 TimeConsumingDto[]
 */
...
/**
 * 热门接口排行
 * 返回 ResultMessage.data 为 HotEndpointDto[]
 */
...
/**
 * 模块分布统计（无参）
 * 返回 ResultMessage.data 为 ModuleDistributionDto[]
 */
...
/**
 * 慢接口排行
 * 返回 ResultMessage.data 为 SlowEndpointDto[]
 */
...
/**
 * 活跃用户排行
 * 返回 ResultMessage.data 为 ActiveUserDto[]
 */
...
/**
 * 操作类型占比（无参）
 * 返回 ResultMessage.data 为 OperateRatioDto[]
 */
...
/**
 * 本周 vs 上周对比（无参）
 * 返回 ResultMessage.data 为 WeeklyCompareDto
 */

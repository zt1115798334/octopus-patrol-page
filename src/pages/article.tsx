...
import { crawlArticles, deleteArticle, findArticlePage, saveArticle } from '@/api/modules/article'
...
export default function ArticleManagement() {
  ...
  const crawlMutation = useMutation({
    mutationFn: crawlArticles,
    onSuccess: () => {
      toast.success('爬取任务已触发')
      queryClient.invalidateQueries({ queryKey: ['articles'] })
    },
    onError: () => {},
  })
  ...
  const handleCreate = useCallback(() => {
    setEditing(null)
    form.reset({ title: '', content: '', url: '', publishTime: '' })
    setDialogOpen(true)
  }, [form])
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => crawlMutation.mutate({ pageNumber: 1, pageSize: 100 })}
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="relative flex-1 max-w-sm">
              ...
              <Input
                ...
                onKeyDown={(e) =>
                  e.key === 'Enter' && queryClient.invalidateQueries({ queryKey: ['articles'] })
                }
              />
            </div>
            <Button
                ...
                {items.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="text-xs text-neutral-500 max-w-[200px] truncate">
                      <a href={item.url || '#'} target="_blank" className="hover:text-primary-500">
                        {item.url}
                      </a>
                    </TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            ...
            <Input label="内容" {...form.register('content')} />
            <Input label="发布时间" type="datetime-local" {...form.register('publishTime')} />
            <DialogFooter>

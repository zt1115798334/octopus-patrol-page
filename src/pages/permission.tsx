...
import { deletePermission, findPermissionPage, savePermission } from '@/api/modules/permission'
import type { PermissionDto, QueryPermissionDto } from '@/types'

const formSchema = z.object({
  name: z.string().min(1, '请输入权限名称'),
  permissionKey: z.string().min(1, '请输入权限标识'),
...
export default function PermissionManagement() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => findPermissionPage(query),
  })
  ...
  const handleCreate = useCallback(() => {
    setEditing(null)
    form.reset({ name: '', permissionKey: '', url: '', httpMethod: 'GET' })
    setDialogOpen(true)
  }, [form])
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="relative flex-1 max-w-sm">
              ...
              <Input
                ...
                onKeyDown={(e) =>
                  e.key === 'Enter' && queryClient.invalidateQueries({ queryKey: ['permissions'] })
                }
              />
            </div>
            <Button
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t('permission.key')}</TableHead>
                  <TableHead>{t('permission.url')}</TableHead>
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="font-mono text-xs">{item.permissionKey}</TableCell>
                    <TableCell className="font-mono text-xs text-neutral-500">{item.url}</TableCell>
                    <TableCell>
                      <Badge variant="info">{item.httpMethod}</Badge>
                    </TableCell>
                    <TableCell>
                      <DropdownMenu>
        ...
        <DialogContent>
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            ...
            <Input
              ...
              error={form.formState.errors.permissionKey?.message}
            />
            <Input label={t('permission.url')} {...form.register('url')} />
            <Input label={t('permission.method')} {...form.register('httpMethod')} />
            <DialogFooter>

...
import {
  changeAiConfigEnabledState,
  deleteAiConfig,
  findAiConfigPage,
  saveAiConfig,
} from '@/api/modules/ai-config'
import type { AiConfigDto, EnabledState, QueryAiConfigDto } from '@/types'
...
export default function AiConfigManagement() {
  ...
  const [editing, setEditing] = useState<AiConfigDto | null>(null)
  const [deleteTarget, setDeleteTarget] = useState<number | null>(null)

  const query: QueryAiConfigDto = { pageNumber: page, pageSize }
  const { data, isLoading } = useQuery({
    queryKey: ['aiConfigs', query],
    queryFn: () => findAiConfigPage(query),
  })
  ...
  const toggleMutation = useMutation({
    mutationFn: changeAiConfigEnabledState,
    onSuccess: () => {
      toast.success(t('common.operationSuccess'))
      queryClient.invalidateQueries({ queryKey: ['aiConfigs'] })
    },
        ...
        <CardContent className="pt-4">
          <div className="flex items-center gap-3 mb-4">
            <Button
              variant="outline"
              size="sm"
              onClick={() => queryClient.invalidateQueries({ queryKey: ['aiConfigs'] })}
            >
          ...
          {isLoading ? (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>{t('aiConfig.name')}</TableHead>
                  <TableHead>{t('aiConfig.vendor')}</TableHead>
                  <TableHead>{t('aiConfig.model')}</TableHead>
                  <TableHead>{t('aiConfig.isDefault')}</TableHead>
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>
                      <Badge variant="info">{vendorLabels[item.vendor || ''] || item.vendor}</Badge>
                    </TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            ...
            />
            <Input label={t('aiConfig.model')} {...form.register('model')} />
            <Input label={t('aiConfig.apiKey')} {...form.register('apiKey')} />
            <Input label={t('aiConfig.apiUrl')} {...form.register('apiUrl')} />
            <DialogFooter>

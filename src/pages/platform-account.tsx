...
import {
  changePlatformAccountEnabledState,
  deletePlatformAccount,
  findPlatformAccountPage,
  savePlatformAccount,
} from '@/api/modules/platform-account'
...
import type { EnabledState, PlatformAccountDto, QueryPlatformAccountDto } from '@/types'
...
export default function PlatformAccountManagement() {
  ...
  const [editing, setEditing] = useState<PlatformAccountDto | null>(null)
  const [deleteTarget, setDeleteTarget] = useState<number | null>(null)

  const query: QueryPlatformAccountDto = { pageNumber: page, pageSize, keywords }
  const { data, isLoading } = useQuery({
    queryKey: ['platformAccounts', query],
    queryFn: () => findPlatformAccountPage(query),
  })
  const form = useForm<FormData>({ resolver: zodResolver(formSchema) })

  const { data: platformsData } = useQuery({
    queryKey: ['onPlatforms'],
    queryFn: () => findOnPlatforms(),
    enabled: dialogOpen,
  })
  const platforms = platformsData?.list || []

  const saveMutation = useMutation({
    mutationFn: savePlatformAccount,
    onSuccess: () => {
  ...
  const toggleMutation = useMutation({
    mutationFn: changePlatformAccountEnabledState,
    onSuccess: () => {
      toast.success(t('common.operationSuccess'))
      queryClient.invalidateQueries({ queryKey: ['platformAccounts'] })
    },
        ...
        <CardContent className="pt-4">
          <div className="flex items-center gap-3 mb-4">
            ...
            <Button
              variant="outline"
              size="icon"
              onClick={() => queryClient.invalidateQueries({ queryKey: ['platformAccounts'] })}
            >
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>
                      <Badge variant="info">{item.platformCode}</Badge>
                    </TableCell>
                    <TableCell>
        ...
        <DialogContent className="max-w-lg">
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <Input
                label={t('platformAccount.account')}
                {...form.register('account')}
                error={form.formState.errors.account?.message}
              />
              <Input
                label={t('platformAccount.password')}
                type="password"
            ...
            <Input label={t('platformAccount.cookie')} {...form.register('cookie')} />
            <div className="grid grid-cols-2 gap-3">
              <Input label={t('platformAccount.accessToken')} {...form.register('accessToken')} />
              <Input label={t('platformAccount.wToken')} {...form.register('wToken')} />
            </div>
            <Input
              label={t('platformAccount.safeUniqueCode')}
              {...form.register('safeUniqueCode')}
            />
            <div className="flex items-center gap-3">
              ...
              <Switch
                ...
                onCheckedChange={(checked) =>
                  form.setValue('enabledState', checked ? 'ON' : 'OFF', { shouldValidate: true })
                }
              />

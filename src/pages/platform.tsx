...
export default function PlatformManagement() {
  ...
  const [editing, setEditing] = useState<PlatformDto | null>(null)
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => findPlatformPage(query),
  })
  ...
  const toggleMutation = useMutation({
    mutationFn: changePlatformEnabledState,
    onSuccess: () => {
      toast.success(t('common.operationSuccess'))
      queryClient.invalidateQueries({ queryKey: ['platforms'] })
    },
  ...
  const handleCreate = useCallback(() => {
    setEditing(null)
    form.reset({ platformName: '', platformCode: '', description: '', enabledState: 'ON' })
    setDialogOpen(true)
  }, [form])
        ...
        <CardContent className="pt-4">
          <div className="flex items-center gap-3 mb-4">
            <Button
              variant="outline"
              size="sm"
              onClick={() => queryClient.invalidateQueries({ queryKey: ['platforms'] })}
            >
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t('platform.description')}</TableHead>
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow key={item.id}>
                    <TableCell className="font-medium">{item.platformName}</TableCell>
                    <TableCell>
                      <Badge variant="info">
                        {platformCodeDescMap[item.platformCode ?? ''] ?? item.platformCode}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell className="text-sm text-neutral-500">
                      {formatDate(item.createdTime, 'yyyy-MM-dd')}
                    </TableCell>
                    <TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            ...
            />
            <Select
              value={form.watch('platformCode') || undefined}
              onValueChange={(v) => form.setValue('platformCode', v, { shouldValidate: true })}
            >

...
import {
  deleteCommentKeyword,
  findCommentKeywordPage,
  saveCommentKeyword,
} from '@/api/modules/comment-keyword'
...
export default function CommentKeywordManagement() {
  ...
  const [editing, setEditing] = useState<CommentKeywordDto | null>(null)
  const [deleteTarget, setDeleteTarget] = useState<number | null>(null)

  const query: QueryCommentKeywordDto = { pageNumber: page, pageSize, keywords }
  const { data, isLoading } = useQuery({
    queryKey: ['commentKeywords', query],
    queryFn: () => findCommentKeywordPage(query),
  })
  const form = useForm<FormData>({ resolver: zodResolver(formSchema) })

  const saveMutation = useMutation({
    mutationFn: saveCommentKeyword,
    onSuccess: () => {
      toast.success(t('common.operationSuccess'))
      setDialogOpen(false)
      queryClient.invalidateQueries({ queryKey: ['commentKeywords'] })
    },
    onError: () => {},
  })
  const deleteMutation = useMutation({
    mutationFn: deleteCommentKeyword,
    onSuccess: () => {
      toast.success(t('common.operationSuccess'))
  ...
  const handleEdit = useCallback(
    (item: CommentKeywordDto) => {
      setEditing(item)
      form.reset({
        keyword: item.keyword || '',
        platformAccountId: item.platformAccountId || undefined,
      })
      setDialogOpen(true)
    },
    [form],
  ...
  const handleCreate = useCallback(() => {
    setEditing(null)
    form.reset({ keyword: '', platformAccountId: undefined })
    setDialogOpen(true)
  }, [form])
        ...
        <CardContent className="pt-4">
          <div className="flex items-center gap-3 mb-4">
            ...
            <Button
              variant="outline"
              size="icon"
              onClick={() => queryClient.invalidateQueries({ queryKey: ['commentKeywords'] })}
            >

...
import { changeTenantEnabledState, deleteTenant, findTenantPage, saveTenant } from '@/api/modules/tenant'
import { formatDate } from '@/lib/utils'
import type { EnabledState, QueryTenantDto, TenantDto } from '@/types'

const formSchema = z.object({
  tenantName: z.string().min(1, '请输入租户名称'),
...
export default function TenantManagement() {
  ...
  const toggleMutation = useMutation({
    mutationFn: changeTenantEnabledState,
    onSuccess: () => {
      toast.success(t('common.operationSuccess'))
      queryClient.invalidateQueries({ queryKey: ['tenants'] })
    },
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="relative flex-1 max-w-sm">
              ...
              <Input
                ...
                onKeyDown={(e) =>
                  e.key === 'Enter' && queryClient.invalidateQueries({ queryKey: ['tenants'] })
                }
              />
            </div>
            <Button
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t('tenant.contact')}</TableHead>
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="font-mono text-xs">{item.tenantCode}</TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            ...
            />
            <Input label={t('tenant.contact')} {...form.register('contactName')} />
            <Input label={t('tenant.contactPhone')} {...form.register('contactPhone')} />
            <Input label={t('tenant.contactEmail')} {...form.register('contactEmail')} />
            <DialogFooter>
